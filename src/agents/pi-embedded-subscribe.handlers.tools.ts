...
export function handleToolExecutionStart(
  ...
  const continueToolExecutionStart = () => {
    ...
    const traceContext = createTraceContext(
      ctx.params.runId,
      ctx.params.sessionId ?? ctx.params.sessionKey ?? "unknown",
      toolName,
      "start",
      toolCallId,
    );
...
export function handleToolExecutionUpdate(
  ...
  const liveEventPartial = isExecToolName(toolName)
    ...
    : sanitized;
  const traceContext = updateTraceContextPhase(ctx.params.runId, "update");
  emitAgentEvent(
    {
      runId: ctx.params.runId,
      stream: "tool",
...
export async function handleToolExecutionEnd(
  ...
  const traceContext = updateTraceContextPhase(ctx.params.runId, "result");
  emitAgentEvent(
    {
      runId: ctx.params.runId,
      stream: "tool",

...
function resolveTraceSearchRoots(): string[] {
  const envRoots = [process.env.DROIDRUN_TRAJECTORY_PATH, process.env.CLAWMOBILE_TRACE_PATH].filter(
...
export default definePluginEntry({
  ...
  register(api: OpenClawPluginApi) {
    ...
    api.registerCommand({
      ...
      acceptsArgs: true,
      handler: async (ctx) => {
        const rawArgs = normalizeOptionalString(ctx.args) ?? "";
        if (!rawArgs) {
        ...
        if (rawArgs.includes(" ")) {
          return {
            text: "Trace match must be exact. Provide exactly one trajectory_id with no extra args.",
          };
        }
        ...
        if (matches.length === 0) {
          return {
            text: `No exact trace match found for ${trajectoryId}.\nNo file was sent.`,
          };
        }
        if (matches.length > 1) {
          return {
